...
//            //其全称为 Uniform Resource Identifier，表示统一资源标识符。您可能已经知道，URL（全称为 Uniform Resource Locator，表示统一资源定位符）是指向某个网页的字符串。URI 是一个更为宽泛的格式术语。所有 URL 都是 URI，但并非所有 URI 都是 URL。
//            context.startActivity(intent)
            val context=holder.view.context
            val intent=Intent(context,problemActivity::class.java)
            intent.putExtra(DetailActivity.SINGPROBLEM,holder.button.text.toString())
            context.startActivity(intent)
        }
